...
import { getRecipeItem, getRecipeItemAuth, getRecipeItemNoAuth } from '@/lib/helpers/server-utils';
...
export default async function SingleRecipePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
   ...
   const recipe = await getRecipeItem<RecipeFull>(
     id,
     session?.backendTokens?.accessToken 
   );
  return <SingleRecipeContainer recipe={recipe} session={session} />;
}
